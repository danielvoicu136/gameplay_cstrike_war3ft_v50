...
#define SH_CONCOCTION_DAMAGE	  10		// Damage done by Unstable Concoction
#define SH_CONCOCTION_RADIUS	  500
	...
	for ( new i = 0; i < numberofplayers; i++ )
	{
		...
		if ( p_data_b[targetid][PB_ISCONNECTED] && get_user_team( targetid ) == iTeam )
		{
			...
			if ( get_distance(origin, targetorigin) < SH_HEALING_WAVE_RANGE )
			{
				...
				// User needs health
				if ( get_user_health( targetid ) + 3 <= get_user_maxhealth( targetid ) )
				{
...
public _SH_DrawSerpentWard( parm[5] )
{
	...
	if ( bWardBombCheck )
	{
		...
		{
			ChatColor( id,"!g* [!t %s!g ] Serpent ward removed, you can't have them near bombs or hostages !", g_MODclient );
	...
	for ( new i = 0; i < numberofplayers; i++ )
	{
		...
		if ( parm[4] != get_user_team( targetid ) )
		{
			...
			if ( get_distance( origin, targetorigin ) < 85 )
			{
				...
				//----------------------------
				if ( ITEM_Has( targetid, ITEM_SNAKE) > ITEM_NONE )
				{
...
public SH_Ult_BigBadVoodoo( id )
{
	...
	if ( SM_GetSkillLevel( id, ULTIMATE_BIGBADVOODOO ) == 1 )
		{ ULT_ResetCooldown( id, 45 + SH_BIGBADVOODOO_DURATION, false ); }	
		else if ( SM_GetSkillLevel( id, ULTIMATE_BIGBADVOODOO ) == 2 )
		{ ULT_ResetCooldown( id, 35 + SH_BIGBADVOODOO_DURATION, false ); }
		else if ( SM_GetSkillLevel( id, ULTIMATE_BIGBADVOODOO ) == 3 )
		{ ULT_ResetCooldown( id, 25 + SH_BIGBADVOODOO_DURATION, false ); }
		else if ( SM_GetSkillLevel( id, ULTIMATE_BIGBADVOODOO ) == 4 )
		{ ULT_ResetCooldown( id, 15 + SH_BIGBADVOODOO_DURATION, false ); }
	...
	if ( g_MOD == GAME_CSTRIKE || g_MOD == GAME_CZERO )
	{
		...
		{Create_BarTime( id, 2*SH_BIGBADVOODOO_DURATION, 0 );}
		else {Create_BarTime( id, SH_BIGBADVOODOO_DURATION, 0 );}
	}
	...
	if (ITEM_Has(id, ITEM_SCEPTER ) > ITEM_NONE)
	{Create_TE_ELIGHT( id, vOrigin, 100, 255, 245, 200, 2*SH_BIGBADVOODOO_DURATION, 0 );}
	else {Create_TE_ELIGHT( id, vOrigin, 100, 255, 245, 200, SH_BIGBADVOODOO_DURATION, 0 );}

	if (ITEM_Has(id, ITEM_SCEPTER ) > ITEM_NONE)
	{set_task( float( 2*SH_BIGBADVOODOO_DURATION ), "SH_Ult_Remove", TASK_RESETGOD + id );}
	else {set_task( float( SH_BIGBADVOODOO_DURATION ), "SH_Ult_Remove", TASK_RESETGOD + id );}
...
SH_SkillsOffensive( iAttacker, iVictim )
{
	...
	if ( iSkillLevel > 0 )
	{
		if ( random_float( 0.0, 1.0 ) <= p_hex[iSkillLevel-1] )
		{
			...
			Create_ScreenFade( iVictim, (1<<10), (1<<10), (1<<12), 255, 255, 255, 100 )
		}
	}
}
...
SH_SkillsDefensive( iAttacker, iVictim )
{
	...
	if ( iSkillLevel > 0 )
	{
		...
		if ( random_float( 0.0, 1.0 ) <= p_concoction[iSkillLevel] )
		{
			...
			for ( i = 0; i < 200; i += 25 )
			{
				Create_TE_BEAMCYLINDER( vOrigin, vInitOrigin, vAxisOrigin, g_iSprites[SPR_SHOCKWAVE], 0, 0, 9, 20, 0, 255, 0, 0, 255, 0 );
...
public _SH_ResetBigBadAttacker( id )
{
	...
	ChatColor( id,"!g* [!t %s!g ] You can no longer damage players with!t Big Bad Voodoo!g activated", g_MODclient );
}