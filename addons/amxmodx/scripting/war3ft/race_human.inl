...
HU_ULT_Blink( id )
{
	...
	if ( iLosesCharge > 0 )
	{
		...
		if ( SM_GetSkillLevel( id, ULTIMATE_BLINK ) == 1 )
		{ ULT_ResetCooldown( id, 45 ); }	
		else if ( SM_GetSkillLevel( id, ULTIMATE_BLINK ) == 2 )
		{ ULT_ResetCooldown( id, 35 ); }
		else if ( SM_GetSkillLevel( id, ULTIMATE_BLINK ) == 3 )
		{ ULT_ResetCooldown( id, 25 ); }
		else if ( SM_GetSkillLevel( id, ULTIMATE_BLINK ) == 4 )
	...
	*/
	...
	// Lets create some beam cylinders!
Create_TE_SPRITE( vOldLocation, g_iSprites[SPR_TELEPORT], 15 , 200 );
...
public _HU_ULT_BlinkStuck( parm[] )
{
	...
	else
	{
		...
		Create_TE_SPRITETRAIL( vStartOrigin, vOrigin, g_iSprites[SPR_FLARE], 30, 10, 2, 50, 10 );
...
HU_ULT_BlinkProtection( id, vOrigin[3] )
{
	...
	if ( bSlay )
	{
		ChatColor( id,"!g* [!t %s!g ] You have been slain for teleporting into a restricted area !", g_MODclient );
		user_kill( id, 1 );
	}
...
HU_SkillsOffensive( iAttacker, iVictim )
{
	...
	if ( iSkillLevel > 0 )
	{
		...
		{		
			...
			// Make the user glow!
			SHARED_Glow( iVictim, 255, 255, 255, BASH_GLOW_INTENSITY );
