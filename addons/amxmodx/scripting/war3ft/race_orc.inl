...
#define CHAINLIGHTNING_DAMAGE			45		// Initial ultimate damage
#define CHAINLIGHTNING_LINEWIDTH		150		// Width of the lightning sprite
...
public OR_ULT_ChainEffect( iCaster, iTarget, iLineWidth, iDamage, iBodyPart )
{
	...
	{
	WC3_Damage( iTarget, iCaster, 2*iDamage, CSW_LIGHTNING, iBodyPart );
	}
	else {
	...
	//Create_TE_BEAMENTS( iCaster, iTarget, g_iSprites[SPR_LIGHTNING], 0, 15, 10, iLineWidth, 10, 255, 255, 255, 255, 0 );
		new echipa, red, green, blue

echipa = get_user_team ( iCaster )
...
if ( g_MOD == GAME_CSTRIKE || g_MOD == GAME_CZERO )
	...
	Create_TE_BEAMENTS( iCaster, iTarget, g_iSprites[SPR_LIGHTNING], 0, 15, 10, iLineWidth, 10, red, green, blue, 255, 0 );
	...
	Create_TE_ELIGHT( iTarget, vOrigin, 100, red, green, blue, 10, 0 );
...
OR_SkillsOffensive( iAttacker, iVictim, iWeapon, iDamage, iHitPlace )
{
	...
	if ( iSkillLevel > 0 )
	{
		...
		if ( random_float( 0.0, 1.0 ) <= CRITICAL_STRIKE_CHANCE )
		{
			...
			// Make the user glow
			SHARED_Glow( iVictim, iBonusDamage, 0, 0, 100 );
