...
WA_ULT_Vengeance( id )
{
	...
	{
		ChatColor( id,"!g* [!t %s!g ] You cannot use your ultimate while planting the bomb !",g_MODclient );
		return;
	}
	...
	if ( !is_user_alive( id ) )
	{
		...
		if ( !p_data[id][P_RESPAWNBY] )
		{
			...
			if ( SM_GetSkillLevel( id, ULTIMATE_VENGEANCE ) == 1 )
		{ ULT_ResetCooldown( id, 45 ); }	
		else if ( SM_GetSkillLevel( id, ULTIMATE_VENGEANCE ) == 2 )
		{ ULT_ResetCooldown( id, 35 ); }
		else if ( SM_GetSkillLevel( id, ULTIMATE_VENGEANCE ) == 3 )
		{ ULT_ResetCooldown( id, 25 ); }
		else if ( SM_GetSkillLevel( id, ULTIMATE_VENGEANCE ) == 4 )
		...
		else
		{
			...
			if (ITEM_Has(id, ITEM_SCEPTER ) > ITEM_NONE)
			{
			 if ( ITEM_Has( id, ITEM_HEALTH ) > ITEM_NONE ) 
                        set_user_health( id, 2*VENGEANCE_HEALTH + get_pcvar_num( CVAR_wc3_health )) 
			 else 
			set_user_health( id, 2*VENGEANCE_HEALTH)
			}
			else {
			 if ( ITEM_Has( id, ITEM_HEALTH ) > ITEM_NONE ) 
                        set_user_health( id, VENGEANCE_HEALTH + get_pcvar_num( CVAR_wc3_health )) 
			 else 
...
WA_Blink( id )
{
	...
	if ( iSkillLevel > 0 )
	{
		...
		if ( random_float( 0.0, 1.0 ) <= p_blink[iSkillLevel-1] )
		{
			...
			ChatColor( id,"!g* [!t %s!g ] You will now be immune to all ultimates !", g_MODclient );
		}
	}
...
WA_SkillsOffensive( iAttacker, iVictim, iHitPlace )
{
	...
	if ( iSkillLevel > 0 )
	{
		...
		if ( random_float( 0.0, 1.0 ) <= p_shadow[iSkillLevel-1] )
		{
			...
			Create_TE_SPRITETRAIL( vAttackerOrigin, vVictimOrigin, g_iSprites[SPR_SHADOWSTRIKE], 50, 15, 2, 2, 6 );
