...
#define CHANCE_ACTIVATION_SIPHONE			0.10
...
public BM_ULT_Immolate( iCaster, iTarget )
{
	...
	{WC3_Damage( iTarget, iCaster, 2*IMMOLATE_DAMAGE, CSW_IMMOLATE, -1 );}
	else { WC3_Damage( iTarget, iCaster, IMMOLATE_DAMAGE, CSW_IMMOLATE, -1 );}
...
public BM_ULT_Immolate_DoT( parm_DoT[3] )
{
    ...
    if ( is_user_alive( iTarget ) )
    {
        ...
        Create_ScreenFade( iTarget, (1<<10), (1<<10), (1<<12), 255, 108, 0, 200 );
...
BM_PhoenixSpawn( id )
{
	...
	if ( iPhoenixID != -1 )
	{
		...
		if ( iPhoenixID != id )
		{
			...
			ChatColor( id,"!g* [!t %s!g ]!t %s !ghas revived you !", g_MODclient, szCastername );
			ChatColor( iPhoenixID,"!g* [!t %s!g ] You have revived!t %s!g !", g_MODclient, szSpawnerName );
		}
	}
...
BM_SkillsOffensive( iAttacker, iVictim, iDamage )
{
	...
	if ( iSkillLevel > 0 )
	{
		...
		if ( random_float( 0.0, 1.0 ) <= CHANCE_ACTIVATION_SIPHONE)
		{
