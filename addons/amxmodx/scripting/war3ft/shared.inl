...
public _SHARED_Spawn( id )
{
	...
	if ( g_EndRound )
	{
		...
		{
			ChatColor( id,"!g* [!t %s!g ] Unable to respawn because the round is over, here is your money back", g_MODclient );
...
public SHARED_SetSpeed( id )
{
	...
	else if ( g_MOD == GAME_DOD )
	{
		...
		else if ( get_user_maxspeed( id ) == 50.0 && ( ITEM_Has( id, ITEM_BOOTS ) > ITEM_NONE || SM_GetSkillLevel( id, SKILL_UNHOLYAURA ) > 0 ) || SM_GetSkillLevel( id, SKILL_RUN ) > 0 )
		{
			set_user_maxspeed( id, 600.0 );

			return;
		}
		else if ( SM_GetSkillLevel( id, SKILL_RUN ) > 0 )
		{
			set_user_maxspeed( id, 600.0 );
	...
	else if ( g_MOD == GAME_CSTRIKE || g_MOD == GAME_CZERO )
	{
		...
		iSkillLevel = SM_GetSkillLevel( id, SKILL_RUN );
		
		// Run bonus
		if ( iSkillLevel > 0.0 )
		{
			fNewSpeed = p_run[iSkillLevel-1];
		}
...
public _SHARED_Mole( parm[2] )
{
	...
	else
	{
		...
		if ( parm[1] == 2 )
		{
			...
			ChatColor( id,"!g* [!t %s!g ] Sorry couldn't find a spot for you to mole, here's your money back", g_MODclient );
		}

		// Moving b/c of skill
		else
		{
			ChatColor( id,"!g* [!t %s!g ] Sorry couldn't find a spot for you to mole", g_MODclient )
		}
	}
...
public _SHARED_Teleport( parm[] )
{
	...
	if ( vOrigin[2] == parm[2] )
	{
		...
		if ( iSpawnEnt > 0 )
		{
			...
			ChatColor( id,"!g* [!t %s!g ] You are stuck! Ahhh! Moving you back to your spawn !", g_MODclient );
		}

		// We can't move the user - that sux0rsz
		else
		{
			ChatColor( id,"!g* [!t %s!g ] Sorry, I know you're stuck, but I can't move you right now ", g_MODclient );
		}
	}
}


 SHARED_GlowShell(id, iRed, iGreen, iBlue,Float:fTime,iAmountGlow = 16) 
 { 
  ...
  // Only glow if the task doesn't exist! 
  else if ( task_exists( TASK_GLOW_SHELL + id ) ) 
   return; 
     
  //set_user_rendering(id, kRenderFxGlowShell,iRed,iGreen,iBlue, kRenderNormal, 16 ); 
  set_user_rendering(id, kRenderFxGlowShell,iRed,iGreen,iBlue, kRenderNormal, iAmountGlow ); 
 
  set_task(fTime, "SHARED_GlowShell_OFF", TASK_GLOW_SHELL + id ); 
 } 
 
 public SHARED_GlowShell_OFF( id ) 
 { 
  if ( id >= TASK_GLOW_SHELL ) 
   id -= TASK_GLOW_SHELL; 
...
SHARED_SetUserHealth( id, health)
{
	if ( !p_data_b[id][PB_ISCONNECTED] )
	{
		return;
