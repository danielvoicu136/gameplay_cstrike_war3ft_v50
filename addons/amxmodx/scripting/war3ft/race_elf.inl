...
public NE_ULT_Entangle( iCaster, iEnemy )
{
	...
	Create_TE_BEAMFOLLOW( iEnemy, g_iSprites[SPR_TRAIL], 10, 5, 255, 255, 255, 255 );
...
if ( g_MOD == GAME_CSTRIKE || g_MOD == GAME_CZERO )
	{
		...
		else
		{
			...
			Create_TE_BEAMPOINTS( iStart, iEnd, g_iSprites[SPR_BEAM], 0, 0, ( floatround( ENTANGLE_TIME ) * 10 ), 35, 5, red, green, blue, 255, 0 );
...
NE_Evasion( id, iHitZone )
{
	...
	{
		new iGlowIntensity = random_num( 30, 70 );
...
NE_SkillsOffensive( iAttacker, iVictim, iWeapon, iDamage, iHitPlace )
{
	...
	{	
		iTempDamage = floatround( float( iDamage ) * p_trueshot_weak[iSkillLevel-1] );
	}
	else iTempDamage = floatround( float( iDamage ) * p_trueshot[iSkillLevel-1] );
