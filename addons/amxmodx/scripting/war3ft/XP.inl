...
public bomb_defusing( defuser )
{
	...
	if ( !bHasBegunPlantingOrDefusing[defuser] )
	{
		...
		{	if((get_user_flags(defuser) & ADMIN_LEVEL_H))     
			{ ChatColor( defuser,"!g* [!t %s!g ]!t  + %d!g XP  ( !tStart Defuse !g )", g_MODclientVIP, iBonusXP ); }
			else ChatColor( defuser,"!g* [!t %s!g ]!t  + %d!g XP  ( !tStart Defuse !g )", g_MODclient, iBonusXP );
...
public bomb_defused( defuser )
{
	...
	if ( iBonusXP != 0 )
	{	if((get_user_flags(defuser) & ADMIN_LEVEL_H))  
		{ChatColor( defuser,"!g* [!t %s!g ]!t  + %d!g XP  ( !tFinish Defuse !g)", g_MODclientVIP, iBonusXP );}
		else {ChatColor( defuser,"!g* [!t %s!g ]!t  + %d!g XP  ( !tFinish Defuse !g)", g_MODclient, iBonusXP );}
	}
...
public bomb_planting( planter )
{
	...
	if ( !bHasBegunPlantingOrDefusing[planter] )
	{
		...
		{	if((get_user_flags(planter) & ADMIN_LEVEL_H))  
			{ChatColor( planter,"!g* [!t %s!g ]!t  + %d!g XP  ( !tStart Plant!g )", g_MODclientVIP, iBonusXP );}
			else {ChatColor( planter,"!g* [!t %s!g ]!t  + %d!g XP  ( !tStart Plant!g )", g_MODclient, iBonusXP );}
		}
	}
...
public bomb_planted( planter )
{
	...
	if ( iBonusXP != 0 )
	{	if((get_user_flags(planter) & ADMIN_LEVEL_H))  
		{ChatColor( planter,"!g* [!t %s!g ]!t  + %d!g XP  ( !tFinish Plant!g )", g_MODclientVIP, iBonusXP );}
		else {ChatColor( planter,"!g* [!t %s!g ]!t  + %d!g XP  ( !tFinish Plant!g )", g_MODclient, iBonusXP );}
	}
...
public on_PlayerAction()
{
	...
	if (equal(sAction,"Spawned_With_The_Bomb"))
	{
		...
		{	if((get_user_flags(id) & ADMIN_LEVEL_H)) 
			{ChatColor( id,"!g* [!t %s!g ]!t  + %d!g XP  ( !tBomb Spawn !g)", g_MODclientVIP, iBonusXP );}
			else {ChatColor( id,"!g* [!t %s!g ]!t  + %d!g XP  ( !tBomb Spawn !g)", g_MODclient, iBonusXP );}
		}
	}
	// Dropped bomb 
	else if (equal(sAction,"Dropped_The_Bomb"))
	{
		...
		if ( iBonusXP != 0 && get_pcvar_num( CVAR_wc3_show_objectives ) )
		{	if((get_user_flags(id) & ADMIN_LEVEL_H)) 
			{ChatColor( id,"!g* [!t %s!g ]!t  - %d!g XP  ( !tBomb Drop !g)", g_MODclientVIP, -1 * iBonusXP );}
			else {ChatColor( id,"!g* [!t %s!g ]!t  - %d!g XP  ( !tBomb Drop !g)", g_MODclient, -1 * iBonusXP );}
	...
	{ 
		...
		if ( iBonusXP != 0 && get_pcvar_num( CVAR_wc3_show_objectives ) )
		{	if((get_user_flags(id) & ADMIN_LEVEL_H)) 
			{ChatColor( id,"!g* [!t %s!g ]!t  + %d!g XP  ( !tBomb Pick!g )", g_MODclientVIP, iBonusXP );}
			else {ChatColor( id,"!g* [!t %s!g ]!t  + %d!g XP  ( !tBomb Pick!g )", g_MODclient, iBonusXP );}
		}
	}
	// Hostage touched 
	else if (equal(sAction,"Touched_A_Hostage"))
	{
		...
		if ( iBonusXP != 0 && get_pcvar_num( CVAR_wc3_show_objectives ) )
		{	if((get_user_flags(id) & ADMIN_LEVEL_H)) 
			{ChatColor( id,"!g* [!t %s!g ]!t  + %d!g XP  ( !tHostage Touche!g )", g_MODclientVIP, iBonusXP );}
			else {ChatColor( id,"!g* [!t %s!g ]!t  + %d!g XP  ( !tHostage Touche!g )", g_MODclient, iBonusXP );}
		}
	}
	// Hostage rescued 
	else if (equal(sAction,"Rescued_A_Hostage"))
	{
		...
		if ( iBonusXP != 0 && get_pcvar_num( CVAR_wc3_show_objectives ) )
		{	if((get_user_flags(id) & ADMIN_LEVEL_H)) 
			{ChatColor( id,"!g* [!t %s!g ]!t  + %d!g XP  ( !tHostage Rescued!g )", g_MODclientVIP, iBonusXP );}
			else {ChatColor( id,"!g* [!t %s!g ]!t  + %d!g XP  ( !tHostage Rescued!g )", g_MODclient, iBonusXP );}
		}
	}
	// Hostage killed 
	else if (equal(sAction,"Killed_A_Hostage"))
	{
		...
		if ( iBonusXP != 0 && get_pcvar_num( CVAR_wc3_show_objectives ) )
		{	if((get_user_flags(id) & ADMIN_LEVEL_H)) 
			{ChatColor( id,"!g* [!t %s!g ]!t  - %d!g XP  ( !tHostage Kill!g )", g_MODclientVIP, -1 * iBonusXP );}
			else {ChatColor( id,"!g* [!t %s!g ]!t  - %d!g XP  ( !tHostage Kill!g )", g_MODclient, -1 * iBonusXP );}
		}
	}
	// VIP spawn 
	...
	{ 
		...
		if ( iBonusXP != 0 && get_pcvar_num( CVAR_wc3_show_objectives ) )
		{	if((get_user_flags(id) & ADMIN_LEVEL_H)) 
			{ChatColor( id,"!g* [!t %s!g ]!t  + %d!g XP  ( !tVIP Spawn!g )", g_MODclientVIP, iBonusXP );}
			else {ChatColor( id,"!g* [!t %s!g ]!t  + %d!g XP  ( !tVIP Spawn!g )", g_MODclient, iBonusXP );}
		}
	}
	// VIP escaped 
	else if (equal(sAction,"Escaped_As_VIP"))
	{
		...
		if ( iBonusXP != 0 && get_pcvar_num( CVAR_wc3_show_objectives ) )
		{	if((get_user_flags(id) & ADMIN_LEVEL_H)) 
			{ChatColor( id,"!g* [!t %s!g ]!t  + %d!g XP  ( !tVIP Escape!g )", g_MODclientVIP, iBonusXP );}
			else {ChatColor( id,"!g* [!t %s!g ]!t  + %d!g XP  ( !tVIP Escape!g )", g_MODclient, iBonusXP );}
		}
	}
}
...
XP_onDeath( iVictim, iAttacker, iWeaponIndex, iHeadshot )
{
	...
	if ( get_user_team( iAttacker ) == get_user_team( iVictim ) )
	{
		...
		{	if((get_user_flags(iAttacker) & ADMIN_LEVEL_H)) 
			{ChatColor( iAttacker,"!g* [!t %s!g ]!t  - %d!g XP  ( !tKill Friend!g )", g_MODclientVIP, -1 * iBonusXP );}
			else {ChatColor( iAttacker,"!g* [!t %s!g ]!t  - %d!g XP  ( !tKill Friend!g )", g_MODclient, -1 * iBonusXP );}
		}
	...
	else
	{
		...
		{	if((get_user_flags(iAttacker) & ADMIN_LEVEL_H)) 
			{ChatColor( iAttacker,"!g* [!t %s!g ]!t  + %d!g XP  ( !tKill!g )", g_MODclientVIP, iBonusXP );}
			else {ChatColor( iAttacker,"!g* [!t %s!g ]!t  + %d!g XP  ( !tKill!g )", g_MODclient, iBonusXP );}
		}
		...
		if ( iHeadshot )
		{
			...
			if ( iBonusXP != 0 && get_pcvar_num( CVAR_wc3_show_kill_obj ) )
			{	if((get_user_flags(iAttacker) & ADMIN_LEVEL_H)) 
				{ChatColor( iAttacker,"!g* [!t %s!g ]!t  + %d!g XP  ( !tHeadShot!g )", g_MODclientVIP, iBonusXP );}
				else {ChatColor( iAttacker,"!g* [!t %s!g ]!t  + %d!g XP  ( !tHeadShot!g )", g_MODclient, iBonusXP );}
			}
		}
		...
		for ( new i = 0; i < MAXPLAYERS; i++ )
		{
			...
			if ( g_iDamageDealt[i][iVictim] > 0 && iAttacker != i )
			{
				...
				{	if((get_user_flags(i) & ADMIN_LEVEL_H)) 
					{ChatColor( i,"!g* [!t %s!g ]!t  + %d!g XP  ( !tKill Assist!g )", g_MODclientVIP, iBonusXP );}
					else {ChatColor( i,"!g* [!t %s!g ]!t  + %d!g XP  ( !tKill Assist!g )", g_MODclient, iBonusXP );}
				}
		...
		if ( g_MOD == GAME_CSTRIKE || g_MOD == GAME_CZERO )
		{
			...
			if ( g_iPlayerRole[iVictim] == PLR_HOSTAGE_RESCUER )
			{
				...
				if ( iBonusXP != 0 && get_pcvar_num( CVAR_wc3_show_kill_obj ) )
				{	if((get_user_flags(iAttacker) & ADMIN_LEVEL_H)) 
					{ChatColor( iAttacker,"!g* [!t %s!g ]!t  + %d!g XP  ( !tKill Hostage Rescuer!g )", g_MODclientVIP, iBonusXP );}
					else {ChatColor( iAttacker,"!g* [!t %s!g ]!t  + %d!g XP  ( !tKill Hostage Rescuer!g )", g_MODclient, iBonusXP );}
				}
			}
			...
			else if ( g_iPlayerRole[iVictim] == PLR_BOMB_DEFUSER )
			{
				...
				if ( iBonusXP != 0 && get_pcvar_num( CVAR_wc3_show_kill_obj ) )
				{	if((get_user_flags(iAttacker) & ADMIN_LEVEL_H)) 
					{ChatColor( iAttacker,"!g* [!t %s!g ]!t  + %d!g XP  ( !tKill Bomb Defuser!g )", g_MODclientVIP, iBonusXP );}
					else {ChatColor( iAttacker,"!g* [!t %s!g ]!t  + %d!g XP  ( !tKill Bomb Defuser!g )", g_MODclient, iBonusXP );}
				}
			}
			...
			else if ( g_iPlayerRole[iVictim] == PLR_BOMB_PLANTER )
			{
				...
				if ( iBonusXP != 0 && get_pcvar_num( CVAR_wc3_show_kill_obj ) )
				{	if((get_user_flags(iAttacker) & ADMIN_LEVEL_H)) 
					{ChatColor( iAttacker,"!g* [!t %s!g ]!t  + %d!g XP  ( !tKill Bomb Planter!g )", g_MODclientVIP, iBonusXP );}
					else {ChatColor( iAttacker,"!g* [!t %s!g ]!t  + %d!g XP  ( !tKill Bomb Planter!g )", g_MODclient, iBonusXP );}
				}
			}
			...
			else if ( g_iPlayerRole[iVictim] == PLR_BOMB_CARRIER )
			{
				...
				if ( iBonusXP != 0 && get_pcvar_num( CVAR_wc3_show_kill_obj ) )
				{	if((get_user_flags(iAttacker) & ADMIN_LEVEL_H)) 
					{ChatColor( iAttacker,"!g* [!t %s!g ]!t  + %d!g XP  ( !tKill Bomb Carrier!g )", g_MODclientVIP, iBonusXP );}
					else {ChatColor( iAttacker,"!g* [!t %s!g ]!t  + %d!g XP  ( !tKill Bomb Carrier!g )", g_MODclient, iBonusXP );}
				}
			}
			// user killed the VIP
			else if ( g_iPlayerRole[iVictim] == PLR_VIP )
			{
				...
				if ( iBonusXP != 0 && get_pcvar_num( CVAR_wc3_show_objectives ) )
				{	if((get_user_flags(iAttacker) & ADMIN_LEVEL_H)) 
					{ChatColor( iAttacker,"!g* [!t %s!g ]!t  + %d!g XP  ( !tKill VIP!g )", g_MODclientVIP, iBonusXP );}
					else {ChatColor( iAttacker,"!g* [!t %s!g ]!t  + %d!g XP  ( !tKill VIP!g )", g_MODclient, iBonusXP );}
				}
			}
		}
...
XP_WinRound( iWinner )
{
	...
	for ( i = 0; i < numberofplayers; i++ )
	{
		...
		if ( get_user_team( id ) == iWinner )
		{
			...
			{	if((get_user_flags(id) & ADMIN_LEVEL_H))			
				{ChatColor( id,"!g* [!t %s!g ]!t  + %d!g XP  ( !tWin Round!g )", g_MODclientVIP, iBonusXP );}
				else {ChatColor( id,"!g* [!t %s!g ]!t  + %d!g XP  ( !tWin Round!g )", g_MODclient, iBonusXP );}
			}
		}
	}
}
...
public XP_Reset(id)
{
	...
	ChatColor( id,"!g* [!t %s!g ] Your XP has been reset to 0 !", g_MODclient );
...
stock XP_Give( id, iBonusXP )
{
	...
	if ( iBonusXP != 0 )
	{
		...
		new Float:iLevelMultiplier = ( fCurrentLevel / 25.0 ) + 1.0;
		new iRealBonusXP = floatround(iLevelMultiplier * iBonusXP);
		
		 if((get_user_flags(id) & ADMIN_LEVEL_H))     
			{
			p_data[id][P_XP] += floatround(iRealBonusXP * 3.0);
			}
			else {  p_data[id][P_XP] += iRealBonusXP; }

		XP_Check( id );
		
		/*
		
		// Afisare In Dhud A XP 
        ...
        if( (1 <= id <= g_iMaxPlayers) && is_user_connected(id) )
        {
            ...
            if( iPos == sizeof(g_flCoords) )
            {
			 ...
			 if((get_user_flags(id) & ADMIN_LEVEL_H))     
			{
            set_dhudmessage(255, 255, 255, Float:g_flCoords[iPos][0], Float:g_flCoords[iPos][1], 2, 0.0, 2.0, 0.02, 0.02)
            show_dhudmessage(id, "VIP XP : %d",iRealBonusXP)
			}
			else 
			{ set_dhudmessage(255, 255, 255, Float:g_flCoords[iPos][0], Float:g_flCoords[iPos][1], 2, 0.0, 2.0, 0.02, 0.02)
			  show_dhudmessage(id, "XP : %d",iRealBonusXP)
			}
        }
