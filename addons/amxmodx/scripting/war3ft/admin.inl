...
ADMIN_SetLevel( id, szTarget[], iLevel )
{
	...
	while ( ( iTarget = FindTarget( iTarget, szTarget ) ) > 0 )
	{
		...
		ChatColor( iTarget,"!g* [!t %s!g ] The admin has set!t your race's level!g to!t %d", g_MODclient, iLevel );
...
ADMIN_GiveXP( id, szTarget[], iXP )
{
	...
	while ( ( iTarget = FindTarget( iTarget, szTarget ) ) > 0 )
	{
		...
		ChatColor( iTarget,"!g* [!t %s!g ] The admin gave you!t %d !gexperience", g_MODclient, iXP);
...
ADMIN_GiveItem( id, szTarget[], iItemID )
{
	...
	while ( ( iTarget = FindTarget( iTarget, szTarget ) ) > 0 )
	{
		...
		ChatColor( iTarget,"!g [!t %s!g ] The admin has given you the item!t '%s'", g_MODclient, szItemName );
