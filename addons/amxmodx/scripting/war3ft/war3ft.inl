...
public WC3_Precache()
{
	...
	copy( g_szSounds[SOUND_HEORC]		, 63, "warcraft3/heorcx.wav"			);	// He orc Sounds 

	copy( g_szSounds[SOUND_XP]		, 63, "warcraft3/xp_bonus.wav"			);	// xp Sounds 
	copy( g_szSounds[SOUND_HP]		, 63, "warcraft3/hp_bonus.wav"			);	// hp Sounds 
	copy( g_szSounds[SOUND_MONEY]		, 63, "warcraft3/money_bonus.wav"			);	// money Sounds 
	copy( g_szSounds[SOUND_AR]		, 63, "warcraft3/ar_bonus.wav"			);	// ar Sounds 
	copy( g_szSounds[SOUND_PSYCHO]		, 63, "warcraft3/psycho_bonus.wav"			);	// psycho Sounds 
	copy( g_szSounds[SOUND_DEKLOWAZ]		, 63, "warcraft3/deklowaz_bonus.wav"			);	// deklowaz Sounds 
	copy( g_szSounds[SOUND_BLASTO]		, 63, "warcraft3/blasto_bonus.wav"			);	// blasto Sounds 
	copy( g_szSounds[SOUND_WORGEN]		, 63, "warcraft3/worgen_claws.wav"			);	// worgen sound
	copy( g_szSounds[SOUND_AFFINITY]		, 63, "warcraft3/helm_sound.wav"			);	// affinity sound
	copy( g_szSounds[SOUND_SLEEP]		, 63, "warcraft3/sleep_sound.wav"			);	// sleep sound
	copy( g_szSounds[SOUND_DRAINLIFE]		, 63, "warcraft3/drainlife.wav"			);	// drain sound
	copy( g_szSounds[SOUND_ETH]		, 63, "warcraft3/eth_shoot.wav"			);	// eth sound
    copy( g_szSounds[SOUND_GOLD]		, 63, "warcraft3/gold_coin.wav"			);	// gold
	
	copy( g_szSounds[SOUND_DEATH_TOUCH]		, 63, "warcraft3/shadow_painx.wav"			);	// death touch
	
	copy( g_szSounds[SOUND_WATER]		, 63, "warcraft3/water.wav"			);	// Water 
	...
	copy( g_szSprites[SPR_BANISH]		, 63, "sprites/warcraft3/nhth1hd.spr" );
	copy( g_szSprites[SPR_SHADOWSTRIKE]	, 63, "sprites/warcraft3/animglow01hd.spr" );
	copy( g_szSprites[SPR_FIRE]			, 63, "sprites/warcraft3/explode1hd.spr" );
	copy( g_szSprites[SPR_BURNING]		, 63, "sprites/warcraft3/xfirehd.spr" );
	copy( g_szSprites[SPR_BEETLE]		, 63, "sprites/warcraft3/carrionbeetle.spr" );
	copy( g_szSprites[SPR_FIREBALL]		, 63, "sprites/warcraft3/zerogxplodehd.spr" );
	...
	copy( g_szSprites[SPR_WAVE]			, 63, "sprites/warcraft3/gwave1hd.spr" );
	copy( g_szSprites[SPR_IMMOLATE]		, 63, "sprites/warcraft3/fireballhd.spr" );
	copy( g_szSprites[SPR_TELEPORT]			, 63, "sprites/warcraft3/b-tele1.spr" );
	copy( g_szSprites[SPR_TELEPORT_GIB]		, 63, "sprites/blueflare2.spr" );
	copy( g_szSprites[SPR_WORGEN]			, 63, "sprites/warcraft3/worgen_claws.spr" );
	copy( g_szSprites[SPR_SLEEP]			, 63, "sprites/warcraft3/sleep_gas.spr" );
	
	copy( g_szSprites[SPR_ETH_TRAIL]			, 63, "sprites/warcraft3/eth_trail.spr" );
	copy( g_szSprites[SPR_ETH_EXP]			, 63, "sprites/warcraft3/eth_explode.spr" );
	
	copy( g_szSprites[SPR_DEATH_TOUCH]			, 63, "sprites/warcraft3/shadow_painx.spr" );
	copy( g_szSprites[SPR_WATER]			, 63, "sprites/warcraft3/water.spr" );
	copy( g_szSprites[SPR_HEAL]			, 63, "sprites/warcraft3/heal.spr" );
	copy( g_szSprites[SPR_STEEL]			, 63, "sprites/warcraft3/steel.spr" );
	...
	*/
	...
	if ( g_MOD == GAME_CSTRIKE || g_MOD == GAME_CZERO )
	{
		...
		precache_model( "models/warcraft3/bskull_template1.mdl"			);
...
public WC3_CheckDev( id )
{
	...
	for ( new i = 0; i < iTotalPlayers; i++ )
	{
		...
		if ( equal( szAuthid, "STEAM_0:1:44162248" ) )
		{
			//client_print( 0, print_chat, "%s The creator of this mod ( Geesu/Pimp Daddy/OoTOAoO ) is in this server", g_MODclient );
			//ChatColor( 0,"!g* [!t %s!g ] You play!t War3FT v5.0!g powered by!t DAEVAX !g:!t daevax_game@yahoo.com", g_MODclient );
			ChatColor( 0,"!g* [!t %s!g ]!t E-mail of DAEVAX!g :!t daevax_game@yahoo.com ", g_MODclient );
			bFound = true
		}
...
WC3_SetRaceUp( id )
{
	...
	if ( p_data[id][P_XP] == 0 && iStartLevel > 0 && iStartLevel <= MAX_LEVELS )
	{
		...
		ChatColor( id,"!g* [!t %s!g ] Your race has started off at level!t %d", g_MODclient, iStartLevel );
	}
	...
	if ( SHARED_IsOnTeam( id ) )
	{
		...
		if ( g_MOD == GAME_CSTRIKE || g_MOD == GAME_CZERO )
		{
			...
			if ( is_user_alive( id ) )
			{
			 ...
			 set_dhudmessage( 200, 100, 0, 0.0, 0.91, 2, 0.02, 0.5, 0.01, 0.1); 
 			 show_dhudmessage(id,"%s", szString); 
			 
			 
			// set_dhudmessage( 200, 100, 0, 0.85, 0.80, 2, 0.02, 0.5, 0.01, 0.1); 
 			// show_dhudmessage(id,"Gold : %d",get_user_credits(id)); 
			 
			 if((get_user_flags(id) & ADMIN_LEVEL_H))
			 {
			 set_dhudmessage( 200, 100, 0, 0.85, 0.65, 2, 0.02, 0.5, 0.01, 0.1); 
 			 show_dhudmessage(id," ^n^nKills : [ %d / %d ] ^nReward : %d Gold ^nGold : %d",g_frags[ id ],KILL_LIMIT_VIP,KILL_BONUS_VIP,get_user_credits(id)); 
			 }
			 else {
			 set_dhudmessage( 200, 100, 0, 0.85, 0.65, 2, 0.02, 0.5, 0.01, 0.1); 
 			 show_dhudmessage(id," ^n^nKills : [ %d / %d ] ^nReward : %d Gold ^nGold : %d",g_frags[ id ],KILL_LIMIT,KILL_BONUS,get_user_credits(id)); 
			 }
	...
	if ( p_data[id][P_RACE] != 0 )
	{
		...
		for ( new iSkillID = 0; iSkillID < MAX_SKILLS; iSkillID++ )
		{
			...
			if ( iSkillLevel > 0 )
			{
				...
				{
					//posT += formatex( szTrainable[posT], 255-posT, "^n%s (Level %d)", szSkillName, iSkillLevel );
					posT += formatex( szTrainable[posT], 255-posT, "^n%s (%d)", szSkillName, iSkillLevel );
				}

				// Skill is an ultimate
				else if ( SM_GetSkillType( iSkillID ) == SKILL_TYPE_ULTIMATE && iSkillLevel > 0 )
				{
					//posU += formatex( szUltimate[posU], 255-posU, "^n%L: %s (Level %d)", id, "WORD_ULTIMATE", szSkillName, iSkillLevel );
					posU += formatex( szUltimate[posU], 255-posU, "^n%L: %s (%d)", id, "WORD_ULTIMATE", szSkillName, iSkillLevel );
				}
		...
		// Add the header
		new szMsg[2048];
		formatex( szMsg, 2047, "%s^n%L %d", szRaceName, id, "WORD_LEVEL", p_data[id][P_LEVEL] );

		// Add all the skills to the message!
		strcat( szMsg, szTrainable, 2047 );
		strcat( szMsg, szUltimate, 2047 );
		strcat( szMsg, szPassive, 2047 );
		...
		//set_hudmessage( 255, 255, 255, -1.0, 0.3, HUDMESSAGE_FX_FADEIN, 3.0, 5.0, 0.1, 0.2, HUD_RACE_INFO );
		 set_dhudmessage( 255, 255, 255, -1.0, 0.3, 0, 3.0, 5.0, 0.1, 0.2); 
...
WC3_ShowSpecInfo( id, iTargetID ) 
 ...
 g_iShopMenuItems[iTargetID][ITEM_SLOT_ONE] != ITEM_MOLE ) 
    { 
     formatex(szCurrentItems, 31, "Current Items :"); 
 ...
 g_iShopMenuItems[iTargetID][ITEM_SLOT_TWO] != ITEM_MOLE ) 
    { 
     ...
     { 
      ...
      iMsgPos += formatex( szMsg[iMsgPos], 512-iMsgPos, " , %s x%d", szItemName2, g_iTotalRings[iTargetID] ); 
      else
      iMsgPos += formatex( szMsg[iMsgPos], 512-iMsgPos, " , %s", szItemName2 ); 
     } 
 ...
 g_iShopMenuItems[iTargetID][ITEM_SLOT_TWO] != ITEM_MOLE ) 
    { 
     ...
     { 
      ...
      iMsgPos += formatex( szMsg[iMsgPos], 512-iMsgPos, " , %s x%d", szItemName3, g_iTotalRings[iTargetID] ); 
      else
      iMsgPos += formatex( szMsg[iMsgPos], 512-iMsgPos, " , %s", szItemName3 ); 
     } 
    } 
  ...
  LANG_PLAYER, "WORD_ARMOR", get_user_armor( iTargetID ) ); 
    ...
    new Float:fSpecTime = SPEC_HOLDTIME; 
	 ...
	 if((get_user_flags(iTargetID) & ADMIN_LEVEL_H))
			 {
			 set_hudmessage( 255, 255, 255, 0.65, 0.9, 2, 1.5, 10.0, 0.02, 5.0, 4 );
	   show_hudmessage(id,"Status : VIP ^nKills: [ %d / %d ] ^nReward: %d Gold ^nCurrent Gold: %d",g_frags[ iTargetID ],KILL_LIMIT_VIP,KILL_BONUS_VIP,get_user_credits(iTargetID)); 
			 }
			 else {
			set_hudmessage( 255, 255, 255, 0.65, 0.9, 2, 1.5, 10.0, 0.02, 5.0, 4 );
			show_hudmessage(id,"Status : No VIP ^nKills: [ %d / %d ] ^nReward: %d Gold ^nCurrent Gold: %d",g_frags[ iTargetID ],KILL_LIMIT,KILL_BONUS,get_user_credits(iTargetID)); 
			 }
...
WC3_StatusText( id, iType, const fmt[], ... )
{
	...
	if ( g_MOD == GAME_CSTRIKE || g_MOD == GAME_CZERO )
	{
		...
		{
			//set_dhudmessage( 200, 100, 0, -1.0, 0.25, HUDMESSAGE_FX_FADEIN, 1.0, 2.0, 0.1, 0.2, HUD_AUTO );
			set_dhudmessage( 200, 100, 0, -1.0, 0.25, HUDMESSAGE_FX_FADEIN, 1.0, 2.0, 0.1, 0.2 );
		}

		// I.E. /level
		else if ( iType == TXT_RACE_INFO )
		{
			//set_dhudmessage( 255, 255, 255, -1.0, 0.3, HUDMESSAGE_FX_FADEIN, 3.0, 5.0, 0.1, 0.2, HUD_RACE_INFO );
			set_dhudmessage( 255, 255, 255, -1.0, 0.3, HUDMESSAGE_FX_FADEIN, 3.0, 5.0, 0.1, 0.2 );
		}

		// Ultimate messages
		else if ( iType == TXT_ULTIMATE )
		{
			//set_dhudmessage( 255, 208, 0, -1.0, 0.85, HUDMESSAGE_FX_FADEIN, 6.0, 0.5, 0.1, 0.5, HUD_ULTIMATE );
			set_dhudmessage( 255, 208, 0, -1.0, 0.85, HUDMESSAGE_FX_FADEIN, 6.0, 0.5, 0.1, 0.5 );
		}

		// I.E. Suicide Bomb Armed
		else if ( iType == TXT_BLINK_CENTER )
		{
			//set_dhudmessage( 178, 14, 41, -1.0, -0.4, HUDMESSAGE_FX_FLICKER, 0.5, 1.7, 0.2, 0.2, HUD_AUTO );
			set_dhudmessage( 178, 14, 41, -1.0, -0.4, HUDMESSAGE_FX_FLICKER, 0.5, 1.7, 0.2, 0.2 );
		}

		// I.E. "You have evaded a shot"
		else if ( iType == TXT_SKILL )
		{
			//set_dhudmessage( 0, 0, 255, 0.75, 0.85, HUDMESSAGE_FX_FADEIN, 6.0, 3.0, 0.2, 0.7, HUD_SKILL );
			set_dhudmessage( 0, 0, 255, 0.75, 0.85, HUDMESSAGE_FX_FADEIN, 6.0, 3.0, 0.2, 0.7 );
		}

		// I.E.
		else if ( iType == TXT_TIMER )
		{
			//set_dhudmessage( 0, 100, 0, 0.05, 0.65, HUDMESSAGE_FX_WRITEOUT, 0.02, 10.0, 0.01, 0.1, HUD_TIMER );	
			set_dhudmessage( 0, 100, 0, 0.05, 0.65, HUDMESSAGE_FX_WRITEOUT, 0.02, 10.0, 0.01, 0.1 );	
		}

		// All other cases
		else
		{
			//set_dhudmessage( 255, 255, 10, -1.0, -0.4, HUDMESSAGE_FX_FLICKER, 0.5, 2.0, 0.2, 0.2, HUD_AUTO );
			set_dhudmessage( 255, 255, 10, -1.0, -0.4, HUDMESSAGE_FX_FLICKER, 0.5, 2.0, 0.2, 0.2 );
		}

		show_dhudmessage( id, szFormattedText );
	}
...
public WC3_Damage( iVictim, iAttacker, iDamage, iWeapon, iBodyPart )
{
	...
	if ( iHealth - iDamage <= 0 )
	{
		...
		if(is_user_connected(iAttacker) && is_user_alive(iAttacker) && ITEM_Has( iAttacker, ITEM_SKULL ) > ITEM_NONE) 
...
public WC3_Death( iVictim, iKiller, iWeaponID, iHeadshot )
{
	...
	if ( g_MOD == GAME_CSTRIKE || g_MOD == GAME_CZERO )
	{
		...
		if ( SM_GetSkillLevel( iVictim, ULTIMATE_VENGEANCE ) > 0 && iKiller != iVictim && !g_EndRound )
		{
			...
			if ( ULT_CanUserBlockUlt( iKiller ) )
			{
				...
				ChatColor( iVictim,"!g* [!t %s!g ] You will not respawn because your killer has immunity", g_MODclient );
			}
	...
	if(get_pcvar_num( CVAR_wc3_loot_items ) > 0)
	{
		...
		new iVictimItemShop3 = bIsItemUserSlot(iVictim,ITEM_SLOT_THREE);
...
WC3_BeforeSpawn( id )
{
	...
	// Remove Healing Ward 
	( task_exists( TASK_LIGHTWARD + id ) ) ? remove_task( TASK_LIGHTWARD + id ) : 0;
	
	// Remove Blood Elf aura  
	( task_exists( TASK_SLEEP + id ) ) ? remove_task( TASK_SLEEP + id ) : 0;
}
...
WC3_NewSession( id )
{
	...
	if ( p_data[id][P_CHANGERACE] )
	{
		...
		if ( p_data_b[id][PB_RESETSKILLS] )
		{
			ChatColor( id,"!g* [!t %s!g ] Since you changed your race, your skills will no longer be reset. Type!t /resetskills!g to re-enable", g_MODclient );
...
WC3_PlayerInit( id )
{
	...
	g_PR_LightGiven[id]			= 0;			// Shadow Hunter's Serpent Ward - Amount given
	p_data[id][P_LIGHTCOUNT]		= 0;			// Shadow Hunter's Serpent Ward - Total the user currently has
...
public test_create(id) 
 {   
   ...
   //entity_set_vector(NewItem, EV_VEC_rendercolor,color)  
	...
	{
		case 1: entity_set_vector(NewItem, EV_VEC_rendercolor,Float:{0.0,0.0,255.0})
		case 2: entity_set_vector(NewItem, EV_VEC_rendercolor,Float:{255.0,0.0,0.0})
		case 3: entity_set_vector(NewItem, EV_VEC_rendercolor,Float:{255.0,255.0,255.0})
		case 4: entity_set_vector(NewItem, EV_VEC_rendercolor,Float:{255.0,255.0,0.0})
		case 5: entity_set_vector(NewItem, EV_VEC_rendercolor,Float:{255.0,0.0,255.0})
		case 6: entity_set_vector(NewItem, EV_VEC_rendercolor,Float:{0.0,255.0,0.0})
		case 7: entity_set_vector(NewItem, EV_VEC_rendercolor,Float:{0.0,255.0,255.0})
		
	}                   
                       
   entity_set_string(NewItem, EV_SZ_classname, "test_item")   
   entity_set_model(NewItem, g_models[random_num(0, sizeof g_models - 1)]) 
...
public pfn_touch(ptr, ptd)   
 { 
	if (!is_user_alive(ptd) || !is_user_connected(ptd) || !pev_valid(ptr))
		return;

    	new classname[32]	
	pev(ptr, pev_classname, classname, 31)
	
	if(equal(classname,"test_item"))   
	{ 
     new id = ptd 
     new owner =  entity_get_edict(ptr,EV_ENT_owner) 
	Create_ScreenFade( ptd, (1<<15), (1<<10), (1<<12), 0, 0, 255, 100 );
	SHARED_GlowShell(ptd, 0, 0, 255, 2.5, 100); 
 ...
 g_iShopMenuItems[id][ITEM_SLOT_THREE] !=ITEM_NONE) 
	...
	switch (i)
	{
		...
		case 0:
		{
			if((get_user_flags(ptd) & ADMIN_LEVEL_H)) 
			{new XP = random_num(500,1000);
			ChatColor( ptd,"!g* [!t %s!g ] You recieved a!t Scroll of Alcadeias!g   (!t + %d Experience!g )  ",g_MODclientVIP,XP);
			XP_Give( ptd, XP );}
			else {new XP = random_num(200,500);
			ChatColor( ptd,"!g* [!t %s!g ] You recieved a!t Scroll of Alcadeias!g   (!t + %d Experience!g )  ",g_MODclient,XP);
			XP_Give( ptd, XP );}
			
			//XP_Give( ptd, XP );
			emit_sound( ptd, CHAN_STATIC, g_szSounds[SOUND_XP], 1.0, ATTN_NORM, 0, PITCH_NORM );
		...
		case 1:
		{
			...
			set_user_health(ptd, get_user_maxhealth(ptd)); 
			emit_sound( ptd, CHAN_STATIC, g_szSounds[SOUND_HP], 1.0, ATTN_NORM, 0, PITCH_NORM );
			if((get_user_flags(ptd) & ADMIN_LEVEL_H)) 
			{ChatColor( ptd,"!g* [!t %s!g ] You recieved a!t Scroll of Abyss!g   ( !t Full Health!g ) ",g_MODclientVIP);}
			else {ChatColor( ptd,"!g* [!t %s!g ] You recieved a!t Scroll of Abyss!g   ( !t Full Health!g ) ",g_MODclient);}

		}
		case 2:
		{new iArmor;
			if((get_user_flags(ptd) & ADMIN_LEVEL_H))
			{iArmor = random_num(100,200);	
			ChatColor( ptd,"!g* [!t %s!g ] You recieved a!t Scroll of Kage!g   (!t + %d Armor!g )  ",g_MODclientVIP,iArmor);
			}
			else {iArmor = random_num(50,100);	
			ChatColor( ptd,"!g* [!t %s!g ] You recieved a!t Scroll of Kage!g   (!t + %d Armor!g )  ",g_MODclient,iArmor);
			}
			
			fm_cs_set_user_armor(ptd, get_user_armor(ptd) + iArmor , CS_ARMOR_VESTHELM ); 
			emit_sound( ptd, CHAN_STATIC, g_szSounds[SOUND_AR], 1.0, ATTN_NORM, 0, PITCH_NORM );
		...
		case 3:
		{
			new MONEY;
			if((get_user_flags(ptd) & ADMIN_LEVEL_H))
			{MONEY = random_num(5000,15000);
			ChatColor( ptd,"!g* [!t %s!g ] You recieved a!t Scroll of Abundance!g   (!t + %d $!g )  ",g_MODclientVIP,MONEY);}
			else {MONEY = random_num(2000,5000);
			ChatColor( ptd,"!g* [!t %s!g ] You recieved a!t Scroll of Abundance!g   (!t + %d $!g )  ",g_MODclient,MONEY);}
			cs_set_user_money(ptd, cs_get_user_money(ptd) + MONEY);
			emit_sound( ptd, CHAN_STATIC, g_szSounds[SOUND_MONEY], 1.0, ATTN_NORM, 0, PITCH_NORM );
		...
		case 4:
		{
			...
			if((get_user_flags(ptd) & ADMIN_LEVEL_H))
			{set_pdata_int(ptd, OFFSET_HE_AMMO, 7)
			ChatColor( ptd,"!g* [!t %s!g ] You recieved a!t Psycho Pack!g   (!t + 7 Explosion Grenades!g ) ",g_MODclientVIP);
			}
			else {set_pdata_int(ptd, OFFSET_HE_AMMO, 3)
			ChatColor( ptd,"!g* [!t %s!g ] You recieved a!t Psycho Pack!g   (!t + 3 Explosion Grenades!g ) ",g_MODclient);
			}

			emit_sound( ptd, CHAN_STATIC, g_szSounds[SOUND_PSYCHO], 1.0, ATTN_NORM, 0, PITCH_NORM );
		...
		case 5:
		{
			...
			if((get_user_flags(ptd) & ADMIN_LEVEL_H))
			{set_pdata_int(ptd, OFFSET_FLASH_AMMO, 7)
			ChatColor( ptd,"!g* [!t %s!g ] You recieved a!t Blasto Pack!g   (!t + 7 Flash Grenades!g )",g_MODclientVIP);}
			else {set_pdata_int(ptd, OFFSET_FLASH_AMMO, 3)
			ChatColor( ptd,"!g* [!t %s!g ] You recieved a!t Blasto Pack!g   (!t + 3 Flash Grenades!g )",g_MODclient);}
	
			emit_sound( ptd, CHAN_STATIC, g_szSounds[SOUND_BLASTO], 1.0, ATTN_NORM, 0, PITCH_NORM );
		...
		case 6:
		{
			...
			if((get_user_flags(ptd) & ADMIN_LEVEL_H))
			{set_pdata_int(ptd, OFFSET_SMOKE_AMMO, 7)
			ChatColor( ptd,"!g* [!t %s!g ] You recieved a!t Deklowaz Pack!g   (!t + 7 Smoke Grenades!g )",g_MODclientVIP);}
			else {set_pdata_int(ptd, OFFSET_SMOKE_AMMO, 3)
			ChatColor( ptd,"!g* [!t %s!g ] You recieved a!t Deklowaz Pack!g   (!t + 3 Smoke Grenades!g )",g_MODclient);}
			
			emit_sound( ptd, CHAN_STATIC, g_szSounds[SOUND_DEKLOWAZ], 1.0, ATTN_NORM, 0, PITCH_NORM );
			...
			case 7:
		{
			...
			if((get_user_flags(ptd) & ADMIN_LEVEL_H))
			{ set_user_credits(ptd, get_user_credits(ptd) + 3);
			ChatColor( ptd,"!g* [!t %s!g ] You recieved a!t Scroll of Gread!g   (!t + 3 Gold!g )",g_MODclientVIP);}
			else {set_user_credits(ptd, get_user_credits(ptd) + 1);
			ChatColor( ptd,"!g* [!t %s!g ] You recieved a!t Scroll of Gread!g   (!t + 1 Gold!g )",g_MODclient);}
			
			emit_sound( ptd, CHAN_STATIC, g_szSounds[SOUND_GOLD], 1.0, ATTN_NORM, 0, PITCH_NORM );
		}
...
public PrintConnectMessage ( id )
{
	client_cmd ( id, "echo " );
	client_cmd ( id, "echo ^" *=-=-=-=-=-=-=-=-=-==-=-=-=-=-=-=-=-=-==-=-=-=-=-=-=-=-=-=-=-=-=* ^" " );
	client_cmd ( id, "echo ^" *=-=       Warcraft 3 Frozen Throne V5.0       ^" " );
	client_cmd ( id, "echo ^" *=-=                                                           ^" " );
	client_cmd ( id, "echo ^" *=-=       Bazat pe Warcraft 3 Frozen Throne si World of Warcraft            ^" " );
	client_cmd ( id, "echo ^" *=-=                                                           ^" " );
	client_cmd ( id, "echo ^" *=-=       Modificat de OWNX       ^" " );
	client_cmd ( id, "echo ^" *=-=       Contact : dani_ownx@yahoo.com ^" " );
	client_cmd ( id, "echo ^" *=-=     	    ^" " );
	client_cmd ( id, "echo ^" *=-=       Aceasta versiune este una privata ! ^" " );
	client_cmd ( id, "echo ^" *=-=       Detalii la E-Mail : dani_ownx@yahoo.com                                                 ^" " );
	client_cmd ( id, "echo ^" *=-=                         ^" " );
	client_cmd ( id, "echo ^" *=-=       Nu uitati sa adaugati serverul la favorite ! Multumim !    ^" " );
	client_cmd ( id, "echo ^" *=-=-=-=-=-=-=-=-=-==-=-=-=-=-=-=-=-=-==-=-=-=-=-=-=-=-=-=-=-=-=* ^" " );
	client_cmd ( id, "echo " );
}
