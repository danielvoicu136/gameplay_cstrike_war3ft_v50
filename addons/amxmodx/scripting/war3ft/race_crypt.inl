...
CL_ULT_LocustGetTarget( id )
{
	...
	if ( ULT_CanUserBlockUlt( iVictim ) )
	{
		...
		if ( SM_GetSkillLevel( id, ULTIMATE_LOCUSTSWARM ) == 1 )
		{ ULT_ResetCooldown( id, 45 ); }	
		else if ( SM_GetSkillLevel( id, ULTIMATE_LOCUSTSWARM ) == 2 )
		{ ULT_ResetCooldown( id, 35 ); }
		else if ( SM_GetSkillLevel( id, ULTIMATE_LOCUSTSWARM ) == 3 )
		{ ULT_ResetCooldown( id, 25 ); }
		else if ( SM_GetSkillLevel( id, ULTIMATE_LOCUSTSWARM ) == 4 )
...
CL_ULT_LocustSwarm( id )
{
	...
	if ( SM_GetSkillLevel( id, ULTIMATE_LOCUSTSWARM ) == 1 )
		{ ULT_ResetCooldown( id, 45, false ); }	
		else if ( SM_GetSkillLevel( id, ULTIMATE_LOCUSTSWARM ) == 2 )
		{ ULT_ResetCooldown( id, 35, false ); }
		else if ( SM_GetSkillLevel( id, ULTIMATE_LOCUSTSWARM ) == 3 )
		{ ULT_ResetCooldown( id, 25, false ); }
		else if ( SM_GetSkillLevel( id, ULTIMATE_LOCUSTSWARM ) == 4 )
		{ ULT_ResetCooldown( id, 15, false ); }
	// Start the effects!!
...
public _CL_ULT_LocustEffect( parm[] )
{
	...
	{
		ChatColor( iAttacker,"!g* [!t %s!g ] Victim is no longer targetable for Locust Swarm, finding new target !", g_MODclient );
		...
		ChatColor( iAttacker,"!g* [!t %s!g ] Victim is no longer targetable , try casting again !", g_MODclient );
	...
	else
	{
		...
		if (ITEM_Has(iAttacker, ITEM_SCEPTER ) > ITEM_NONE)
		{
		WC3_Damage( iVictim, iAttacker, 2*iDamage, CSW_LOCUSTS, -1 );
		}
		else { WC3_Damage( iVictim, iAttacker, iDamage, CSW_LOCUSTS, -1 ); }
		...
		ChatColor( iAttacker,"!g* [!t %s!g ] Locust Swarm hit!t %s !gfor!t %d !gdamage !", g_MODclient, szName, iDamage );
	}
}
...
CL_SkillsDefensive( iAttacker, iVictim, iDamage, iHitPlace )
{
	...
	if ( iSkillLevel > 0 )
	{
		...
		if ( is_user_alive( iAttacker ) )
		{
			...
			// Make the user glow!
			SHARED_Glow( iAttacker, ( 4 * iTemp ), 0, 0, 0);
